...
$('#ownbutton').on('click',function(){
  $('#promptbtn').show();
  let personal_prompts=global_api_response.filter((item)=>item.user ===2);
...
$.ajax({
    ...
    method:'POST',
    data:{template,teaser,hint,title,author,url,topic,activity,tone,public,file_content,user:2},
    success:function(){
...
<select style="border-radius: 5px;
  ...
  <option value="Improve">Improve</option>
   ...
   <div> <b style="font-size: 12px;
    ...
    margin-left: 45px;margin-top:5px;position:fixed"> Fine Tuning </b>
    </div>
